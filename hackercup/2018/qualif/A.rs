...
fn main() {
    ...
    for case in 1..=cases {
        let (n, k, v): (usize, usize, usize) = read_line(&mut buffer);
        let attractions: Vec<String> = read_lines_iter(&mut buffer, n).collect();

        print!("Case #{}:", case);

        let start_i = ((v - 1) * k) % n;
        let end_i = (start_i + k) % n;

        if end_i <= start_i {
            for name in &attractions[0..end_i] {
                print!(" {}", name);
            }

            for name in &attractions[start_i..n] {
                print!(" {}", name);
            }
        } else {
            for name in &attractions[start_i..end_i] {
                print!(" {}", name);
            }
        }
