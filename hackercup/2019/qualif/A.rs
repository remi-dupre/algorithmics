...
fn main() {
    ...
    for case in 1..=cases {
        let config: String = read_line(&mut buffer);

        let nb_lilis = config.len();
        let nb_betas = config.bytes().filter(|&c| c == b'B').count();
        let result = {
            if nb_lilis == 1 || nb_betas + 1 < nb_lilis && 2 * nb_betas >= nb_lilis - 1 {
                'Y'
            } else {
                'N'
            }
