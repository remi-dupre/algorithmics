...
fn match_next(expr: &[u8], x_val: bool) -> (bool, &[u8]) {
    match expr[0] {
        b'1' => (true, &expr[1..]),
        b'0' => (false, &expr[1..]),
        b'x' => (x_val, &expr[1..]),
        b'X' => (!x_val, &expr[1..]),
        b'(' => {
            let (left, expr) = match_next(&expr[1..], x_val);
            let sign = expr[0];
            let (right, tail) = match_next(&expr[1..], x_val);
...
fn main() {
    ...
    for case in 1..=cases {
        ...
        let result = {
            if match_next(expr.as_bytes(), true).0 == match_next(expr.as_bytes(), false).0 {
                0
            } else {
                1
            }
