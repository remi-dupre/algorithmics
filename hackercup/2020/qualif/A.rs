...
fn reach(from: usize, input: &[bool], output: &[bool]) -> RangeInclusive<usize> {
    let mut start = from;
    let mut end = from;

    while start > 0 && output[start] && input[start - 1] {
        start -= 1;
    }

    while end + 1 < input.len() && output[end] && input[end + 1] {
        end += 1;
    }
...
fn main() {
    ...
    for case in 1..=cases {
        ...
        let input: Vec<_> = read_line::<String>(&mut buffer)
            .bytes()
            .map(|x| x == b'Y')
            .collect();

        let output: Vec<_> = read_line::<String>(&mut buffer)
            .bytes()
            .map(|x| x == b'Y')
            .collect();
        ...
        for departure in 0..n {
            let reach = reach(departure, &input, &output);

            for dest in 0..n {
                print!("{}", if reach.contains(&dest) { "Y" } else { "N" });
            }
