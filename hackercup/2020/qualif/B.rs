...
fn main() {
    ...
    for case in 1..=cases {
        ...
        let shards: String = read_line(&mut buffer);

        let a = shards.bytes().filter(|x| *x == b'A').count() as isize;
        let b = shards.bytes().filter(|x| *x == b'B').count() as isize;
        assert_eq!(a + b, n);

        println!(
            "Case #{}: {}",
            case,
            if (a - b).abs() == 1 { 'Y' } else { 'N' }
        );
    }
}
