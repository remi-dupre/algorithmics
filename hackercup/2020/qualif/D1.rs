...
fn main() {
    ...
    for case in 1..=cases {
        ...
        let stations: Vec<i64> = read_lines_iter(&mut buffer, n).collect();
        let mut cost_for_full: BinaryHeap<_> = once((0, 0)).collect();

        for (curr_pos, curr_cost) in stations.into_iter().enumerate() {
            while cost_for_full
                .peek()
                .map(|(_, pos)| curr_pos - pos > m)
                .unwrap_or(false)
            {
                cost_for_full.pop();
            }
            ...
            if curr_cost != 0 {
                if let Some(&(cost, _)) = cost_for_full.peek() {
                    cost_for_full.push((cost - curr_cost, curr_pos))
                }
            }
        }
        ...
        println!(
            "Case #{}: {}",
            case,
            cost_for_full.peek().map(|(cost, _)| -cost).unwrap_or(-1)
        );
    }
}
